...
Upstream sibling project is a web app named `kxyy_ai_clone`; the `ai/` logic modules, persona corpus, and pet/sticker assets are **synced from it** and must stay contract-compatible (see Sync below). Desktop-only modules (`realtime.js`, `pcm-worklet.js`) are **not** part of that sync.
...
There is no test suite and no linter configured. Build artifacts land in `src-tauri/target/release/bundle/`.
...
- **Frontend side** owns audio: `getUserMedia` (with AEC/noise-suppression — essential since the pet is a speaker/output scenario prone to echo-feedback), an AudioWorklet (`src/ai/pcm-worklet.js`) resampling mic to 16k s16le PCM upstream, and Web Audio scheduling of 24k PCM downstream with barge-in (flush on ASR).
...
If upstream changes the `/api/chat` request/response contract, manually update **both** `src-tauri/src/api.rs` and `src/chat.js`. After `sync-ai` (which overwrites only `persona.js` / `tts.js` / `persona-assets.js` / `stickers.js` + sticker assets — **not** `realtime.js` / `pcm-worklet.js` / `avatars.js`), re-run `npm run encrypt-assets`.
