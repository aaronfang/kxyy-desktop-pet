...
`scripts/local-realtime/vad_adapter.py` provides bounded 512-sample assembly, probability hysteresis/generation/fallback, and the 0.2.25 `VadShadowWorker`; `acoustic_manifest.py` remains a development validator for strict synthetic-only provenance. `vad_adapter.py` is a Tauri resource and `common.py::Session` can inject it as a dedicated daemon worker, but production `run()` passes no factory: diagnostic v2 reports `vadShadow=disabled`, starts no thread, and the live path remains the existing 480-sample RMS VAD. Shadow admission is process-wide 1, waiting queue 1; overflow must advance epoch/reset continuity, and late results never drive Session events. Do not describe this as neural VAD shipping, feed RMS values into the probability API, add a hard Silero/ONNX Runtime dependency, or accept public recordings without separate schema/license/consent review. Any real scorer must remain capability-gated, off the WebSocket event loop and `_mlx_pool`, with RMS in parallel until acoustic replay validates fallback and a bounded candidate timeout.
