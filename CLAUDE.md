...
`scripts/local-realtime/vad_adapter.py` and `acoustic_manifest.py` are 0.2.24 development/test foundations only: bounded 512-sample PCM assembly, injected probability hysteresis/generation/fallback, and a strict synthetic-only provenance validator. `common.py::Session` does not import them and Tauri does not bundle them as runtime resources yet; the live path remains the existing 480-sample RMS VAD. Do not describe this as neural VAD shipping, feed RMS values into the probability API, add Silero/ONNX Runtime, or accept public recordings without a separate schema/license/consent review. Any future live scorer must run on a dedicated single worker with queue=1, never on the WebSocket event loop or `_mlx_pool`, and must keep RMS processing in parallel until acoustic replay validates fallback and a bounded candidate timeout.
