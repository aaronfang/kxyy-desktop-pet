...
- **Diagnostics** stay frontend-only and in memory. With `showChatDebug`, the chat can copy the current or most recently stopped call as a bounded `diagnosticSchemaVersion:3` JSON report. It re-sanitizes at most 256 events and 8 generation latency summaries, aliases all ids, includes only fixed runtime capability enums, monotonic relative timing, and allow-listed numeric metrics, and never includes keys, persona, text, URLs/paths, device names, or PCM. Consecutive playback stats coalesce while preserving the sampled queue maximum and a coalesced count. `maxSampledQueuedMs` reflects 500ms observations, while `drainInclusiveUnderruns` includes natural drain; neither is a provider-internal streaming metric.
...
Local realtime LLM output is SSE with a bounded 32-event Python queue and at most two blocking producers/TTS tasks. A pure `StableSentenceBuffer` feeds a bounded 4-sentence ordered TTS pipeline; playback is always single-lane/in-order. Qwen synthesis parallelism stays 1; macOS MLX TTS and barge-in ASR share `_mlx_pool`. With negotiated `provider-pcm-v1`, MLX pulls one official 0.32-second generation result at a time, rejects any abnormal result over 2 seconds, and does not prefetch the next `next()` during sender pacing; Windows/Linux PyTorch has no public audio iterator and remains buffered. CosyVoice streaming uses a 2-item receive queue; both streaming adapters disable next-sentence prefetch. Buffered Qwen/Cosy results remain capped at 60 seconds, and HTTP TTS admission is 2 tasks whose slots release only when the actual Future finishes. Official Cosy examples support PCM24k but do not normatively document byte order; keep its desktop s16le interpretation marked as real-account validation pending. Blocking socket/TTS/MLX `next()` calls cannot be force-killed, so every return/send boundary must re-check the generation CancelScope, and all new queues/admission paths must stay bounded.

0.2.26 bundles the unmodified Silero VAD v6.2.1 16k/op15 ONNX model, strict manifest, MIT license, and a real recurrent scorer in `silero_shadow.py`. It remains opt-in (`vadShadowEnabled=false` by default) and shadow-only: RMS is still the sole candidate/endpoint/ASR decision path. ONNX Runtime is never a core requirement or automatic startup install; the settings button runs `install_vad_runtime.py`, which selects only the audited CPython/macOS/Windows matrix, downloads four exact wheel specs into an App-data staging directory, validates allow-listed wheel hashes plus a real model inference, writes the ready marker last, and publishes a versioned ABI target through a recoverable sibling swap that retains the old target until the new rename succeeds. The Rust manager serializes settings-driven ensure, installer stop/restart, and macOS setup completion under one lifecycle lock with desired backend/fingerprint/epoch state; stale background completions must never override a newer backend choice. Unsupported, missing, warming, busy, or faulted states keep RMS working and expose only fixed `vadShadow` enums. The process-wide worker starts after voice `prepare()`, uses admission=1/queue=1 and monotonic lease tokens; stale owners cannot offer/release into a new Session. Never add a hard `silero-vad`, Torch, `onnxruntime-gpu`, source-build, or floating dependency; changes to the model/runtime lock require re-verifying upstream tag, commit, byte size, SHA-256, license, exact wheel filenames/hashes, and real smoke. Neural results must not drive live interruption before licensed acoustic replay and bounded candidate timing are validated.

Local/CosyVoice `managed-v1` is negotiated by `start.downlinkAudio` and the `session` response; if either side does not select it, downlink remains raw PCM for backward compatibility. Its fixed 24-byte big-endian `KXAU` v1 header carries generation, segment id, strict chunk sequence, and payload sample count; chunks are at most 80ms/1920 samples and a segment is at most 750 chunks/60s. Buffered segments declare exact samples at start. CosyVoice or supported macOS MLX Qwen + Worklet may additionally negotiate `start.ttsStream` / `session.ttsStream` as `provider-pcm-v1`; only then may start omit unknown samples, while completed end must declare exact final samples and chunks. Failed/cancelled/mismatched streams are dropped and never produce an audible completion receipt. The frontend accepts only the current generation and registered segment, sequence starting at zero with no reorder buffer; all pending queues and ledgers stay at existing 64-item limits. The mirrored constants in `scripts/local-realtime/common.py` and `src/ai/realtime.js` must change together with deterministic tests. `KXAU` is not a Volcano constant: never infer from it or modify `realtime.rs::protocol` because of it.
