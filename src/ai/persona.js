...
/** 非 kxyy（skill 卡 / 非主播角色）日常在线提示：通用、中性，不注入直播概念。 */
function offAirNoteGeneric() {
  return [
    "- **日常聊天模式**：你就是你自己，用你一贯的性格和方式跟对面聊天。",
    "  像朋友之间发微信一样自然、松弛——别端着、别像客服，也别刻意套近乎。",
  ];
}
...
export function buildMessages({
  ...
  if (proactiveKind) {
    ...
    const isKxyy = isKxyyPersona(cardId);
    runtimeMsgs.push({ role: "system", content: deep ? deepTalkHint(isKxyy) : REPLY_FORMAT_HINT });
...
export function saveCardVoice(cardId, voice) {
  ...
  if (voice && (voice.realtimeBackend || voice.ttsVoice || voice.cosyvoiceVoice)) {
