...
| 近期互联网话题 | 可选的中国大陆生活/兴趣来源预取、统一短期缓存，以及文字/本地实时语音按需注入 |
...
设置中的“近期话题”页可单独开启或关闭统一缓存。开启后，应用启动会异步采集固定的中文来源；缓存只保存有限的标题、短摘要、来源、链接和时间，不写入长期记忆，也不会把聊天、persona 或设备信息发送给来源。设置页可以按分类查看条目、来源状态和短缺原因，并手动重新采集。

文字聊天和本地/CosyVoice 实时通话共用这份缓存。文字聊天可以选择“相关时带一句”“偶尔找个话头”或“更主动一点”：后两档只按固定低频从本机缓存抽取一条合适线索，不会让每句话都联网，也不会在同一会话反复播报同一来源。用户明确询问或请求推荐时仍会按分类取用，例如“推荐几首歌”“最近有什么新游戏”“哪里有好吃的”；普通的“我最近在看书”之类第一人称近况不会被当成主动推荐触发。火山实时语音和旧版本地服务不会注入这份缓存。

默认来源包括中新网 RSS、百度热榜、哔哩哔哩热门、IT之家 RSS、Steam/Epic/App Store 游戏来源、豆瓣电影、汽水音乐/网易云音乐、城市餐饮搜索、携程餐厅榜/景点榜和必应分类搜索；来源可用性受网络、限流和页面格式变化影响，设置中的“可用”只代表最近一次请求可解析，不代表每个分类每次都能填满配额。
...
| VoxCPM2 本地（实验） | Apple Silicon / Windows 上测试零样本克隆音色 | 首次选择时安装运行时与模型 |

实时通话入口在聊天输入框最左侧的电话按钮，对所有人设卡可用。通话中会暂时禁用文字输入、图片和表情库；可将聊天窗收起为可拖动的置顶通话胶囊，拖到屏幕左右边缘后会自动收窄，宽度可在设置中调整。第一次使用需要允许麦克风权限。
...
苗疆元元的本地语音设置页还提供 6 个发布 preset：自动评分最高的 5 个参考音，以及固化为 `legacy-12s.wav/.txt` 的最早内置基线。当前默认音独立使用新的 `ref.wav/ref.txt`。选择菜单项后 Python 服务会在下一句合成前热加载，不会重启服务；手动填写参考音频会覆盖菜单选择。发布包只包含新默认音、5 个评分参考音、旧基线和清单，75 条试听样例及其余候选只保留在本地忽略目录。
