...
- **Memory v3/v3.1（v0.2.43 正式版）**：长期记忆由 Rust + SQLite 保存为事实、共同经历、约定和可追溯事件；聊天前只选择性召回与当前话题相关的少量内容，后台异步巩固，不再把全部历史塞进 prompt。设置页可搜索、筛选、编辑、置顶、兑现、永久删除或查看关系图；数据库或模型不可用时会回退，不阻塞正常聊天。
- **0.2.43 集成版**：Memory v3/v3.1、关系图、实时通话记忆协调、默认关闭的 Workspace 基础和主动通话问候已合入正式基线；Windows 本地 Qwen3-TTS 默认切换为 0.6B，首次下载超过 180 秒后继续显示加载并在服务就绪时自动恢复。
...
- **Memory Brain 路线**：后续实时通话逐轮记忆延迟验收、Global Workspace/J-Space 类实验和外部工程接入，统一见 [`docs/roadmap-memory-brain.md`](docs/roadmap-memory-brain.md)。未标记为 released 的阶段不作为当前正式版能力承诺。
...
- **Windows / Linux（本地 Qwen3-TTS）**：走官方 PyTorch 包 `qwen-tts`。Windows 默认加载较低延迟的 `Qwen/Qwen3-TTS-12Hz-0.6B-Base`；Linux 暂时保持 `Qwen/Qwen3-TTS-12Hz-1.7B-Base`。两者首次运行都会从 Hugging Face 官方站点下载相应权重，且当前仍按稳定句整段生成，不等于真流式。下载可能持续数分钟；180 秒后 App 会继续显示下载/加载中并保持健康探测，模型就绪后自动恢复，不会仅因超过固定时限报启动失败。Windows 运行 `scripts/windows/setup-qwen3-tts.cmd` 会创建独立环境 `scripts/local-realtime/.venv-qwen3` 并安装 torch + qwen-tts 等依赖（新环境按 GPU 自动选 wheel：RTX 50 系/Blackwell 用 `cu128`，其它 NVIDIA 用 `cu124`，无卡则 CPU，较慢；Python 需 3.10–3.13，3.14 暂无 wheel）。可在 `settings.json` 用 `qwen3ModelDir`（本地权重目录或模型 id）、`qwen3Language`（默认 `Auto`）覆盖；显式配置始终优先于平台默认。需要自定义 Hugging Face 镜像时可另设 `hfEndpoint`，App 不再默认依赖第三方镜像。
- **RTX 50 系升级提示（v0.2.43 已知限制）**：重新安装 App 会保留用户数据目录中的 `.venv-qwen3`；如果旧环境装过 `torch + cu124`，安装脚本选择 `cu128` 时仍可能因 pip 判定“已安装”而没有升级，随后报 `no kernel image is available for execution on the device`。这不是模型或 Hugging Face 镜像故障。完全退出 App 后，在该环境执行 `python -m pip install --upgrade --force-reinstall torch torchaudio --index-url https://download.pytorch.org/whl/cu128`；验证 `torch.cuda.get_arch_list()` 含 `sm_120` 且真实 CUDA tensor 运算成功后再启动语音服务。安装器自动迁移与真实 CUDA smoke 仍待后续修复。
