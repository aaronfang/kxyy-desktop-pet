...
可恢复播放与候选让声已经有测试版：本地链路检测到疑似人声会立即发 candidate 并由前端 duck/暂停，约 1.05 秒 soft-end/reopen 后才提交整段 Whisper。0.2.17 为本地/CosyVoice 稳定句加入实际播放完成回执；0.2.18 又把 LLM 消费、句级合成和有序播放拆成有界阶段；0.2.19 进一步让本地/CosyVoice 协商 `managed-v1` 下行身份，每个 PCM chunk 携带 generation、segment 和严格递增 sequence；0.2.20 为实际启用播放 Worklet 的本地/CosyVoice 增加 candidate-bound 播放快照：同一 candidate 确认打断、当前登记句段已播放至少 1 秒时，只给下一次 LLM 请求注入一次固定临时提示。火山、旧服务与 legacy 播放均安全降级为无提示。当前主要瓶颈仍是 confirmed 等待句尾与整段 ASR，以及每个稳定句的音频仍需整段合成；神经 VAD、真实声学回放、字/音素级恢复和真正音频流式仍是后续工作。
...
| 本地实时语音 | WebView PCM -> Python -> RMS VAD -> Whisper -> 桌面文字代理 -> TTS | 旁路采集、ASR 验证后打断、Qwen/CosyVoice、generation 取消、DeepSeek/Ollama 复用、LLM SSE、有界句级预取、有序播放、句级可听历史、`managed-v1` 下行身份与 Worklet-only 一次性 interrupted hint | ASR 仍为整段；TTS 仍按稳定句整段合成；神经 VAD、字/音素级恢复与真正音频流式未实现 |
...
无需真实账号、麦克风或模型的验证命令为 `npm test` 与 `npm run test:python`。固定事件夹具覆盖正常问答、候选误触恢复、确认打断后旧 generation 音频拒绝、断线重连后的旧 session 隔离、合成 PCM、SSE/分句/生产生命周期、播放完成/clear/overflow/挂起排序/可听历史回放、`managed-v1` 的固定 header/协商/严格身份校验，以及 `candidate-snapshot-v1` 的 Worklet/legacy 协商、candidate id、1 秒阈值、clear 后迟到快照、50ms timeout、one-shot 与 history 隔离。PR CI 会运行这些命令。
...
2. 固定 2 秒判停、LLM SSE、句级 audible history、有界有序 TTS consumer、本地/CosyVoice managed identity 与一次性 interrupted hint 已完成测试切片；下一步分别验证 CosyVoice、Qwen 的官方真流式 adapter，同时继续收集 trace 决定何时投入神经 VAD。
