...
可恢复播放与候选让声已经有测试版：本地链路检测到疑似人声会立即发 candidate 并由前端 duck/暂停，约 1.05 秒 soft-end/reopen 后才提交整段 Whisper。0.2.17 又为本地/CosyVoice 稳定句加入实际播放完成回执，下一轮上下文不再假定未播尾句已被用户听到。当前主要瓶颈仍是 confirmed 等待句尾与整段 ASR，以及每个稳定句的音频仍需整段合成；神经 VAD、真实声学回放和真正音频流式仍是后续工作。
...
| 本地实时语音 | WebView PCM -> Python -> RMS VAD -> Whisper -> 桌面文字代理 -> TTS | 旁路采集、ASR 验证后打断、Qwen/CosyVoice、generation 取消、DeepSeek/Ollama 复用、LLM SSE、稳定句 TTS 与句级可听历史 | ASR 仍为整段；TTS 仍按稳定句整段合成；神经 VAD 与真正音频流式未实现 |
...
- 默认暂停容量 3 秒仍是测试期上限，正常排队目标仍为 120–250ms；本地裸 PCM wire payload 尚未携带 generation。
...
| `llm_response` | assistant 流结束 | 本地顺序消费会包含稳定句 TTS/PCM 反压等待，不冒充上游模型内部完成时间 |
...
| `response_started` / `response_cancelled` / `response_completed` | ASR 结束、确认插话、挂断或 TTS 生产结束后播放 drain | 本地控制/scoped sender 已有后端 generation；火山和裸 PCM wire 仍未贯穿 |
...
P0 落地时的设计占位 `speech_candidate`、`speech_rejected`、`response_completed` 已在后续 P1 测试版接入运行时，详见下一节。0.2.14 已让本地级联控制事件和发送任务携带后端 generation，0.2.16 已接入 LLM SSE；但火山二进制帧和本地裸 PCM wire payload 仍没有 generation，前端继续用确认后的 audio gate 隔离旧尾包。神经 VAD、真正音频流式和火山后端 generation 仍属于 P2 或待实验范围。

无需真实账号、麦克风或模型的验证命令为 `npm test` 与 `npm run test:python`。固定事件夹具覆盖正常问答、候选误触恢复、确认打断后旧 generation 音频拒绝、断线重连后的旧 session 隔离、合成 PCM、SSE/分句/生产生命周期，以及 0.2.17 的播放完成/clear/overflow/挂起排序/可听历史回放。PR CI 会运行这些命令。
...
边界必须明确：规则波形只能证明状态机和阈值没有机械回归，不能代表真实人声、房间回声、神经 VAD 准确率或“30 分钟误打断 <= 1 次”体验指标。真实或公开许可录音的声学回放集仍待建立。generation-tagged `CancelScope`、文字 provider、LLM SSE/稳定句和句级可听历史已在 0.2.14–0.2.17 依次补齐；下一项基础工作是真正音频流式、有界并行 TTS 或神经 VAD 实验，须按实测 trace 决定顺序。
...
已实现边界不能扩大解释：`run_in_executor` 中已经开始的 Python 阻塞线程无法被强制终止，只是其返回值会在 active checkpoint 被丢弃；一次已经进入 WebSocket `send()` 的裸 PCM chunk 可能成为至多一个在途尾包，binary wire payload 本身尚未携带 generation，仍由 scoped sender、generation 控制事件和 confirmed audio gate 联合阻断后续旧音频。火山端到端路径未增加或修改任何协议常量，也尚未获得同样的后端 generation。0.2.17 已修复本地/CosyVoice 的完整回复 history 污染；句中音素/字级位置和自动打断反馈话术仍是后续设计。
...
0.2.15 当时仍是整段 LLM 基础版：Python 使用 `stream: false`，取消中的 HTTP 请求只能在返回后由 0.2.14 CancelScope 丢弃。该限制中的 LLM SSE 与稳定句已由 0.2.16 取代，句级 audible history 已由 0.2.17 补齐；TTS 并行消费与真正音频流式仍未实现。Ollama 长时间空闲后的 keep-alive 继续复用桌面代理现有的 native touch。
...
纯状态 `StableSentenceBuffer` 以 `。！？!?；;` 和换行为强句末，达到 40 字时优先在 `，,、：:` 切分，60 字无断点时硬切，内部缓冲始终有界；短尾段在流结束时 flush。每个有效 content delta 立即作为 `assistant` delta 发送，首个稳定句在完整回复结束前触发现有 TTS；`tts_start` / `tts_end` 显式包住本轮后端音频生产期，前端只有在生产结束且播放 ring 已 drain 后才记录 response completed。Qwen/CosyVoice 仍是“每个稳定句整段合成 -> 既有 PCM chunk 发送”，**不是真正音频流式**。当前 consumer 为最小安全的顺序实现：合成和发送一句时暂不消费后续 token，32 项队列满后会对上游施加反压，因此 `llm_response` trace 可能包含 TTS/PCM 等待，不能解释成纯模型耗时。
...
无需账号、麦克风或模型的固定测试覆盖：SSE delta/usage、reasoning 隔离与兼容回退、满队列取消、6/40/60 分句、cancel 后不再产出、assistant delta/end、稳定句 TTS、usage 和旧 generation PCM 竞态。手测时选择本地或 CosyVoice 通话：用户气泡定稿后，助手文字应逐步出现，第一句有完整标点时应在后续文字尚未全部出现前开始播音；打断后不应继续出现旧文字或旧音频。0.2.17 对可听 history 的新增范围见下一节；真正音频流式、并行 sentence worker 和真实声学指标仍待实现/实验。

### 2.11 P2 句级可听历史基础版（已实现，0.2.17）

本地/CosyVoice 每个稳定句在发送 PCM 前登记有界 `generation + segmentId`，播放 Worklet 在该句全部 source samples 实际消费完后回报完成；legacy source-node 只在该句所有 source 正常结束后回报。AudioContext 挂起时 PCM 与 segment-end 共用同一个最多 64 项的有序暂存队列，避免控制帧越过尚未入 ring 的音频。Worklet 的 ring、span 与 segment ledger 均有界；clear、ring/暂存溢出或未知/旧 generation 不产生完成回执。
...
这是**句级保守语义**，不是字/音素级对齐：被打断的当前句整句不进入 `audibleText`，即使开头可能已经播放；英文极端 hard-split 的空白边界也不承诺逐字符重建。`interrupted` 已在内存 turn 状态标记，但“播放超过约 1 秒才注入隐藏提示”的反馈话术尚未实现，不能把本版描述为完整 interrupted-history UX。手测可让助手生成两句，在第二句中插话后追问“刚才说到哪”；下一轮可以使用完整播完的第一句，但不应引用第二句未播尾部。
...
| LLM | DeepSeek/Ollama SSE；稳定句 TTS 时顺序反压 | OpenAI-compatible、本地/云端、流式 | 已复用现有 DeepSeek/Ollama provider；后续分离有界 producer/consumer |
| TTS | 稳定句级整段合成后发送 PCM chunk | 句子合并、流式 chunk、TTFA/RTF | 后续引入真正 Qwen/Cosy 音频流式输出 |
| 取消 | generation CancelScope 覆盖 SSE event、逐句 TTS 与 PCM checkpoint；阻塞线程不可强杀 | generation-tagged `CancelScope` | 后续让二进制音频也携带 generation |
...
| 测试 | reducer、合成 PCM、SSE、分句与取消回放；真实许可录音待补 | VAD、取消、生命周期、TTS 合并测试 | 下一步补真实声学回放和体验指标 |
...
P0 trace schema 已采用下列事件语义；candidate/rejected/completed 与 confirmed audio gate 已在 P1/P2 测试版运行，统一 responseId/generation 的二进制音频 envelope 仍是目标：
...
本地控制事件和 scoped sender 已关联后端 generation，前端 trace 另有 responseId；火山后端事件与两条链路的裸 PCM payload 仍未携带同一标识。后续音频 envelope 完成后，取消的旧 generation 输出应由协议层直接丢弃。
...
- LLM 每个 delta、TTS 返回点与 PCM chunk 已检查取消域；神经 VAD 和真正 TTS 音频 chunk 流仍待实现。

### 4.6 对话历史

0.2.17 已让本地/CosyVoice history 区分生成展示与句级可听上下文：

- `generatedText`：模型生成的完整文本，用于气泡展示，不再直接写入本地级联下一轮上下文。
- `audibleText`：前端确认完整播完的连续稳定句，用于下一轮上下文；当前句中打断时整句保守排除。
- `interrupted`：内存 turn 已标记是否取消，但尚未实现字级位置或自动反馈提示。

后续打断反馈不应每次都抱怨。只有上一轮已播放超过约 1 秒且在句中被截断时，下一轮才应注入隐藏提示：“上一轮被打断，不要假设未播尾句已被用户听到；可按人设简短接话，但不要机械复述。”这条提示与字/音素级位置仍是设计占位，不属于 0.2.17 已实现范围。
...
- **待实现**：Qwen/CosyVoice 真正音频流式下发、有界并行 TTS consumer；字/音素级中断位置与按播放时长触发的隐藏反馈仍待实现。
...
2. 固定 2 秒判停、LLM SSE 与句级 audible history 已完成测试切片；下一步优先做有界并行 TTS consumer，为后续后端专用真流式 adapter 建立顺序/取消基础，同时继续收集 trace 决定何时投入神经 VAD。
