...
设计占位（尚未上线）：`speech_candidate`、`speech_rejected`、`response_completed` 已进入 schema 和确定性回放测试，但当前后端没有可靠暴露这些边界，运行时不会伪造；reducer 能识别旧 generation / 旧 session 音频并给出拒绝决定，但 P0 不用该决定 gate 真实播放。generation 贯穿后端、可恢复播放 Ring Buffer、两阶段打断、神经 VAD 和 provider 切换仍分别属于 P1/P2 或待实验范围。

无需真实账号、麦克风或模型的验证命令为 `npm test`。固定事件夹具覆盖正常问答、候选误触恢复、确认打断后旧 generation 音频拒绝、重连后旧 session 迟到事件隔离；PR CI 会运行同一命令。
...
- **设计占位**：candidate/rejected、response completed 与旧 generation gate 已进入 schema/reducer，但尚未驱动当前播放行为。
- **待实验**：建立可合法分发的固定 PCM 回放素材集，覆盖正常说话、短停顿、咳嗽、键盘、外放回声和连续插话；P0 当前测试使用无 PCM 的固定事件夹具。

### P1：可恢复播放与两阶段打断

- 上线播放 AudioWorklet、队列深度统计和正确重采样。
- 扩展 candidate/confirmed/rejected 事件。
- 火山和本地后端分别接入两阶段打断；保留开关以便快速回退到旧策略。
- 通过本节体验指标后，才降低本地句尾等待。

### P2：本地低延迟管线

- 神经 VAD + adaptive endpoint + soft-end/reopen。
- ASR 去临时 WAV，优先使用内存数组接口。
- DeepSeek/Ollama 使用统一 OpenAI-compatible adapter；本地通话遵循 `textProvider`。
- LLM SSE、按稳定句送 TTS、Qwen/CosyVoice 音频流式下发。
- generation-tagged `CancelScope` 覆盖全管线。
