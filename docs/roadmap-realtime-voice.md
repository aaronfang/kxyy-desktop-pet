...
长通话中的“今天不直播/后来又开播”等易变角色状态使用固定、仅会话内的短期状态。12 条 / 4096 字恢复历史会为最新明确状态保留一个槽位，新服务从用户消息重建，不写入长期 Memory。模型在用户未道别时还会收到本地后端统一的持续对话约束；已审核的主动收线尾句在进入 TTS 前过滤，流式聊天草稿通过当前 generation 的 `assistant_replace` 收敛到实际播报文本。用户明确要睡、离开、挂断或道别时仍可自然告别，诊断不保留任何回复正文。
