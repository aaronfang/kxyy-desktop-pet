...
## 0.2.52 搜索上下文注入与实时等待（2026-08-30）

- 明确搜索请求可通过 `web-observation-v1` 注入本地实时通话。
- 上下文等待覆盖 Tavily 有界请求时间，避免模型在搜索完成前生成无联网兜底。
- 搜索来源、结果数和耗时进入聊天 Debug 展示。
...
恢复连接继续使用有界聊天历史、可听句段回执和原有 Memory 队列。未播助手草稿不会进入历史；已播前缀保留；最后一轮用户在尚未出声时可通过显式协商的 `pending-turn-resume-v1` 合并最多四段、512 字后只重新回答一次。前端不会缓存无界 PCM，恢复期间明确暂停接收麦克风输入，完成新 session 握手后继续原话题。

长通话中的“今天不直播/后来又开播”等易变角色状态使用固定、仅会话内的短期状态。12 条 / 4096 字恢复历史会为最新明确状态保留一个槽位，新服务从用户消息重建，不写入长期 Memory。模型在用户未道别时还会收到本地后端统一的持续对话约束；回复正文不做正则截断，避免破坏引用、翻译、例句等正常内容，聊天显示、实际播报与可听历史保留同一份原始回复。用户明确要睡、离开、挂断或道别时仍可自然告别，诊断不保留任何回复正文。

确定性 Python、JavaScript、Rust 与资源契约测试均已覆盖；真实通话已验证普通打断后的回复可以正常出声。强制 provider cleanup timeout、VoxCPM 进程替换和恢复后继续原话题仍需一次受控真机测试。详细不变量与剩余验证见 [Realtime Call Recovery](./spec-realtime-call-recovery.md)。

## 0.2.48 VoxCPM2 macOS 实验支持
...
0.2.26 固定分发未经修改的 Silero VAD `v6.2.1`、上游 commit `7e30209a3e901f9842f81b225f3e93d8199902b1` 的 `silero_vad_16k_op15.onnx`。manifest 锁定 1,289,603 bytes、SHA-256 `7ed98ddbad84ccac4cd0aeb3099049280713df825c610a8ed34543318f1b2c49`、MIT license、16kHz、512 新 samples 与 64 context samples；scorer 另固定并在运行时验证 recurrent state `(2,1,128)`。scorer 只启用 CPU provider，ORT inter/intra-op thread 均为 1；每帧严格校验输入契约以及输出 shape、dtype、finite 和 `[0,1]` 范围。模型、manifest、license 与第三方说明是安装包资源，但 ONNX Runtime 不是核心 Python requirements，也不会在服务启动时自动安装。

**已实现**：设置新增默认关闭的 `vadShadowEnabled` 与显式“安装 / 检查 VAD runtime”动作。安装器只接受审核过的 macOS arm64/x64、Windows amd64 与 CPython 3.10–3.14 精确矩阵；按 `vad-runtime-lock.json` 的固定包版本、wheel 文件名和 SHA-256 下载 binary-only/no-deps wheel，在 App 数据目录的 sibling staging 中安装并做一次真实 recurrent 推理，再最后写 ready marker。发布使用可恢复的 sibling swap：旧 target 保留到新目录 rename 成功，失败回滚，进程中断留下的单一 backup 会在下次安装先恢复。Rust manager 用同一 lifecycle lock 串行化 settings-driven ensure、安装前条件停服、完成后条件恢复与 macOS 首次 Qwen setup 回调，并在锁内维护 desired backend/fingerprint/单调 epoch；用户中途切换后端时，旧后台任务不能再覆盖新选择。导入只从该 target 解析模块，不执行 `.pth`，也不覆盖 Qwen/Whisper 的 voice 环境。unsupported、未安装、损坏或验证失败都使用固定状态降级，不泄露下载/异常详情、路径或环境信息，也不阻止 RMS 通话。
...
### 2.24 P2 SenseVoice final ASR 实验后端（已实现，0.2.30；默认 Whisper）

**已实现——选择与回退**：本地 Qwen/CosyVoice 增加 `whisper|sensevoice` 固定设置枚举；火山端到端链路不变。服务进程只在启动时选择一次 final ASR，不双跑、不按轮切换。SenseVoice runtime 可用时直接从内存 PCM16LE 做整句识别；缺失、marker/布局校验失败、平台不支持或启动加载失败时，本次进程固定回退既有 Whisper。两条路径共用 512 字和长周期重复护栏。ASR admission 为 1，第二个请求不会进入无界 executor 队列；取消后的阻塞结果必须在 Future 真正结束后才释放 slot，且迟到 generation 不得写回会话。
...
### 4.7 主动语音与“元元带聊”模式（本地/CosyVoice A-D 已实现）
...
**2026-08-23 主导感与开场修正**：`ai-leads` 将“不知道聊/做什么”“没安排”“你来讲/推荐”“你有什么新鲜事”等有限交棒表达识别为一次性 `handoff` 软意图，本轮直接选择 `associate + lead`，不等待静默计时；它不是持久状态，也不覆盖暂停、换题、敏感话题或明确建议/观点请求。安全的 `neutral/settling` 日常分支连续两次 `support` 后，下一轮轮换为有依据的 `opine/contrast/lead`，主动问题和敏感分支不参与。每次通话只选择一个 `warm-direct|context-first|playful|topic-first` 开场枚举，主动欢迎和第一条接通确认共用该枚举；Python 再次白名单校验，枚举不进入诊断或历史。普通实时回复以 3--5 句为目标，主动欢迎/续说/换题以 2--3 句为目标，并明确禁止用用户刚说的数字、名词或结论做无信息首句复述；这些是长度目标，不允许用重复、总结或连续提问凑字数。

**2026-08-23 交棒识别兜底**：固定 `handoff` 规则只作为高置信快速通道；每次带真实用户文本的 `ai-leads` 生成请求还附带一次性语义判断提示，识别未命中固定说法的“今天你当主持人”“我负责听，你负责说”“你随便挑个话头”等隐含交棒。该判断与正常回复共用同一次 LLM 请求，不增加分类请求或网络往返；暂停、明确换题、敏感/严肃内容和真实追问仍由本地硬规则优先，模型不能覆盖。若语义判断确认交棒，角色需先自行选择具体方向并连续贡献至少两个推进点，再留低负担入口，不能把选题任务反问回用户。

**2026-08-23 静默接话升级阶梯**：实时通话的静默阈值按“短接话 → 换题 → 停止”处理。一次正常用户回合播完后，按原策略等待 4/6/8 秒做同题低负担续说；如果该续说仍完全无人回应，再等待固定 9 秒做一次 `idle` 换题。换题播完仍无人回应时，本次通话不再主动发言，直到收到新的有效用户回合；任何有效用户回合都会清零静默主动次数和最近主动类型，重新进入正常等待。`ai-leads` 的静默路径最多一次换题，避免因持续无声循环独白；`balanced` 仍保持单次主动上限，`follow-user` 不主动接话。该状态只存在于当前通话，不进入历史、记忆或诊断。

**2026-08-23 回复收尾看门狗**：managed 本地服务新增 `response-finish-v1` 协商。前端收到 `assistant_end` 后，仅在 TTS 已开始但迟迟没有 `tts_end`、当前 generation 的所有句段均有播放完成回执、Worklet 队列已排空且没有用户输入/新音频时，启动固定 12 秒无进展看门狗。超时后发送无文本 `response_finish_recover`，服务端只取消仍卡住的旧 generation，并返回 `response_finish_recovered`；前端确认后才把本轮标记为可收尾并进入正常静默主动计时。用户重新开口、错误、重连或挂断会取消看门狗；迟到旧音频因 generation/segment 校验被丢弃。诊断仅增加固定的 `finishStalls`/`finishRecoveries` 计数，不记录文本、错误或 provider payload。旧服务或未协商该能力时保持原行为。

| 设置值 | 用户体验 | 主动行为 |
|---|---|---|
| `follow-user`（跟随用户） | 用户掌控话题 | AI 只回应，不因静默自行开口 |
| `balanced`（自然轮流） | 双方轮流推进 | 接通问候或用户轮次后至多一次同题续说，不连续换题 |
| `ai-leads`（元元带聊） | AI 负责准备和延续话题，用户可主要附和 | 更快主动开场，最多三个连续主动回合；短附和继续当前话题，长静默最多换题一次 |
...
记忆候选继续使用 Memory observation 格式，不能成为系统指令；不确定信息要以试探口吻确认，不能编造元元的线下经历。`topicKey` 最长 64 字符，和最多 8 项的 `topicsUsed` 都只存在于当前通话，不进入协议、诊断或持久化。
...
| 类别 | 示例 | 行为 |
|---|---|---|
| `acknowledge` | 嗯、哦、好的、明白了 | 表示已听到，补一个具体细节，不连续追问 |
| `amused` | 哈哈、笑死、太逗了 | 顺着轻松情绪接一句，不重复笑声凑回应 |
| `curious` | 是吗、然后呢、怎么说 | 直接补最相关信息，再留低负担回应口 |
| `agree` | 对、没错、确实、有道理 | 承接并推进新细节，不扩写成用户未说过的观点 |
| `resume` | 你继续、接着讲、可以继续了 | 恢复被暂停或因两次负反馈停止的主动调度 |
...
计时起点必须是同 generation **最后一个音频句段的实际播放完成回执**，不是 `assistant_end`、`tts_end`、首个句段回执或 producer 完成。当前固定值如下，真实设备听感仍待验证：

| 触发 | `balanced` | `ai-leads` |
|---|---:|---:|
| 接通后问候候选 | 1.2–2.0s | 0.6–1.2s |
| 同话题续说候选 | 10s | 4.5s |
| 换新话题候选 | 不主动换题 | 14s，最多一次 |
| 连续主动回合上限 | 1 | 2–3 |

前端调度器负责用户设置、窗口/会话状态、话题候选、Memory recall 和冷却；语音后端做最后安全门。后端只有在 `in_speech=false`、无 candidate、无 ASR、无活跃 reply、无播放和无 pending playback receipt 时才能接受。任何 `speech_candidate` 都立即撤销主动候选；节奏负反馈只在 confirmed 后提交，rejected candidate 不改变等待。一次负反馈把后续等待乘 1.5；两个不同主动 trigger 的负反馈使本通停止主动调度，明确 `resume` 清除该停止状态。
...
server: {type:"proactive_turn_status", triggerId, state:"accepted|vetoed|cancelled", reason?:"speech|asr|reply|playback|receipt|cooldown|limit"}
```

协议保留的 `kind` 为 `welcome|followup|idle|memory|commitment`；当前只发送并接受 `welcome|followup|idle`，其余保留值固定 veto。`triggerId` 单调且会话内幂等；旧服务或未回显能力时不得发送。主动回合创建正常 response generation，复用现有 SSE、稳定句、有界 TTS、managed PCM 和 Worklet，不新建第二条播放管线。
...
| 后端 | 首版支持范围 | 边界 |
|---|---|---|
| 本地 Qwen / CosyVoice 级联 | 接通问候、播放后同题续说、ai-leads 一次换题、确认态节奏退让 | 仅显式 `managed-v1 + local-v1`；旧服务、legacy playback 降级为 `none` |
...
3. **C：同题续说（已实现）**——以实际 playback completed 为计时起点；`followup` 使用一次性控制提示，不伪造用户输入。
4. **D：元元带聊（已实现，设备体验待测）**——四类短回应和 `pause/redirect/resume` 固定规则、有界 TopicLeadState、最多一次换题、3 条/300 字 Memory observation、确认态 1.5 倍退让与两次停止。
5. **E：火山能力门**——只做官方协议验证；无证据继续 `none`。

除既有延迟/打断指标外，schema v8 新增固定 trigger kind、互动类别、veto reason、节奏退让/停止计数，以及既有 accepted/vetoed/cancelled、抢回/早期打断、主动回合和 topic switch 数。验收覆盖 39 条镜像中文分类、暂停/恢复、两次负反馈停止、多句段最终回执、话题/Memory ID 上限、旧端/火山降级、挂断迟到 generation 和恶意正文注入后的隐私剥离。不能记录话题正文、用户/助手文本、Memory 内容或候选 ID。
...
- **已实现（0.2.30 实验后端）**：本地/CosyVoice 可显式选择并安装 SenseVoiceSmall INT8 final ASR；ABI/hash/marker 锁定、真实 smoke、单 admission、启动期固定 Whisper 回退已落地；后续主动带聊观测把整体诊断 schema 升至 v8。它仍是整句识别，不驱动 VAD/endpoint/partial；默认值和火山链路不变。
