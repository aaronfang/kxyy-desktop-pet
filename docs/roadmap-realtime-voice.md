...
可恢复播放与候选让声已经有测试版：本地链路检测到疑似人声会立即发 candidate 并由前端 duck/暂停，约 1.05 秒 soft-end/reopen 后才提交整段 Whisper。当前主要瓶颈是 confirmed 仍等待句尾与整段 ASR，以及稳定句虽可提前触发 TTS、每句音频仍需整段合成。神经 VAD、真实声学回放和真正音频流式仍是后续工作。
...
| 本地实时语音 | WebView PCM -> Python -> RMS VAD -> Whisper -> 桌面文字代理 -> TTS | 旁路采集、ASR 验证后打断、Qwen/CosyVoice、generation 取消、DeepSeek/Ollama 复用、LLM SSE 与稳定句 TTS | ASR 仍为整段；TTS 仍按稳定句整段合成；神经 VAD 与真正音频流式未实现 |
...
无需真实账号、麦克风或模型的验证命令为 `npm test` 与 `npm run test:python`。固定事件夹具覆盖正常问答、候选误触恢复、确认打断后旧 generation 音频拒绝、重连后旧 session 迟到事件隔离；0.2.13 又补充了合成 PCM 场景回放，0.2.16 补充 SSE/分句/生产生命周期，详见对应小节。PR CI 会运行这些命令。
...
边界必须明确：规则波形只能证明状态机和阈值没有机械回归，不能代表真实人声、房间回声、神经 VAD 准确率或“30 分钟误打断 <= 1 次”体验指标。真实或公开许可录音的声学回放集仍待建立。generation-tagged `CancelScope` 已在 0.2.14 补齐，DeepSeek/Ollama provider 复用已在 0.2.15 补齐，LLM SSE 与稳定句切分已在 0.2.16 补齐；下一项基础工作是真正音频流式、并行 TTS/audible history 或神经 VAD 实验，须按实测 trace 决定顺序。
...
0.2.15 当时仍是整段 LLM 基础版：Python 使用 `stream: false`，取消中的 HTTP 请求只能在返回后由 0.2.14 CancelScope 丢弃。该限制中的 LLM SSE 与稳定句合并已由 0.2.16 取代；TTS 并行消费、真正音频流式与 audible history 仍未实现。Ollama 长时间空闲后的 keep-alive 继续复用桌面代理现有的 native touch。
...
取消和隐私边界：完整 SSE 成功且 scope 仍 active 才写 history；取消或错误中的 partial 不写完整 history、usage 或后续 PCM。已经阻塞在 socket read 或已经开始的 TTS 线程无法强杀，最坏分别等待网络超时或后端返回，但并发准入和有界队列阻止继续扩张；后台异常会被静默取走并只向前端返回固定安全文案，裸 PCM 仍可能有至多一个在途 chunk。队列只在内存中短暂持有 delta，不记录 secret、原始 PCM、persona 或完整用户/助手文本；日志仅含单调耗时、token/字符数和字节数。

无需账号、麦克风或模型的固定测试覆盖：SSE delta/usage、reasoning 隔离与兼容回退、满队列取消、6/40/60 分句边界、cancel 后不再产出、assistant delta/end、稳定句 TTS、history/usage 以及旧 generation PCM 竞态。手测时选择本地或 CosyVoice 通话：用户气泡定稿后，助手文字应逐步出现，第一句有完整标点时应在后续文字尚未全部出现前开始播音；打断后不应继续出现旧文字或旧音频。真正 Qwen/CosyVoice 音频流式、并行 sentence worker、`audibleText` 中断历史和真实声学指标仍是待实现/待实验项。
...
本地链路当前在完整 SSE 结束后把 generated full reply 写入 history；此前的稳定句可能已经进入播放，而未带强句末的尾句可能尚未合成。history 仍不能区分实际可听文本和中断位置，后续应区分：
...
- **待实现**：Qwen/CosyVoice 真正音频流式下发、有界并行 TTS consumer、`audibleText` 中断历史。
...
2. 固定 2 秒判停与 LLM SSE 已完成测试切片；下一步依据 0.2.16 trace，在真正音频流式/有界并行 TTS 与神经 VAD 之间选择收益更高者，再更换 ASR 模型。
