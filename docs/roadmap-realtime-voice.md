...
可恢复播放与候选让声已经有测试版：本地链路检测到疑似人声会立即发 candidate 并由前端 duck/暂停，约 1.05 秒 soft-end/reopen 后才提交整段 Whisper。0.2.17 为本地/CosyVoice 稳定句加入实际播放完成回执；0.2.18 又把 LLM 消费、句级合成和有序播放拆成有界阶段；0.2.19 进一步让本地/CosyVoice 协商 `managed-v1` 下行身份，每个 PCM chunk 携带 generation、segment 和严格递增 sequence，前端用登记句段与精确样本总量拒绝旧代、错序或残缺音频。火山与旧服务仍走 raw PCM 兼容路径。当前主要瓶颈仍是 confirmed 等待句尾与整段 ASR，以及每个稳定句的音频仍需整段合成；按播放时长触发的 interrupted UX、神经 VAD、真实声学回放和真正音频流式仍是后续工作。
...
| 本地实时语音 | WebView PCM -> Python -> RMS VAD -> Whisper -> 桌面文字代理 -> TTS | 旁路采集、ASR 验证后打断、Qwen/CosyVoice、generation 取消、DeepSeek/Ollama 复用、LLM SSE、有界句级预取、有序播放、句级可听历史与 `managed-v1` 下行身份 | ASR 仍为整段；TTS 仍按稳定句整段合成；interrupted UX、神经 VAD 与真正音频流式未实现 |
...
P0 落地时的设计占位 `speech_candidate`、`speech_rejected`、`response_completed` 已在后续 P1 测试版接入运行时，详见下一节。0.2.14 已让本地级联控制事件和发送任务携带后端 generation，0.2.16 已接入 LLM SSE，0.2.19 又让协商后的本地/CosyVoice 下行 binary 携带同一 generation。火山二进制帧与 raw fallback 仍没有该身份，前端继续用确认后的 audio gate 隔离旧尾包。神经 VAD、真正音频流式和火山后端 generation 仍属于 P2 或待实验范围。

无需真实账号、麦克风或模型的验证命令为 `npm test` 与 `npm run test:python`。固定事件夹具覆盖正常问答、候选误触恢复、确认打断后旧 generation 音频拒绝、断线重连后的旧 session 隔离、合成 PCM、SSE/分句/生产生命周期、播放完成/clear/overflow/挂起排序/可听历史回放，以及 `managed-v1` 的固定 header、协商降级、旧/未来 generation、未知句段、错序/重复/缺失、样本总量和恢复路径。PR CI 会运行这些命令。
...
边界必须明确：规则波形只能证明状态机和阈值没有机械回归，不能代表真实人声、房间回声、神经 VAD 准确率或“30 分钟误打断 <= 1 次”体验指标。真实或公开许可录音的声学回放集仍待建立。generation-tagged `CancelScope`、文字 provider、LLM SSE/稳定句、句级可听历史、有界有序 TTS consumer 和本地/CosyVoice 下行 identity 已在 0.2.14–0.2.19 依次补齐；下一项基础工作是 interrupted UX，之后才进入后端专用真流式实验。
...
已实现边界不能扩大解释：`run_in_executor` 中已经开始的 Python 阻塞线程无法被强制终止，只是其返回值会在 active checkpoint 被丢弃；一次已经进入 WebSocket `send()` 的本地/CosyVoice chunk 仍可能成为至多一个在途尾包，但 0.2.19 的 managed header 会让前端按 generation 拒绝其进入当前句段。未协商的 raw fallback 仍由 scoped sender、generation 控制事件和 confirmed audio gate 联合阻断后续旧音频。火山端到端路径未增加或修改任何协议常量，也尚未获得同样的后端 generation。0.2.17 已修复本地/CosyVoice 的完整回复 history 污染；句中音素/字级位置和自动打断反馈话术仍是后续设计。
...
| 测试 | reducer、合成 PCM、SSE、分句、有界 TTS 顺序/取消、managed identity；真实许可录音待补 | VAD、取消、生命周期、TTS 合并测试 | 下一步补 interrupted UX，再补后端真流式与真实声学指标 |
...
P0 trace schema 已采用下列事件语义；candidate/rejected/completed 与 confirmed audio gate 已在 P1/P2 测试版运行。0.2.19 已让本地/CosyVoice binary 使用 generation/segment/chunk identity；统一 responseId 与火山路径仍是目标：
...
0.2.19 的 managed chunk identity 只是后续播放进度/中断关联的基础，仍不按 chunk 比例推断已听文本。后续打断反馈不应每次都抱怨。只有上一轮已播放超过约 1 秒且在句中被截断时，下一轮才应注入一次性隐藏提示：“上一轮被打断，不要假设未播尾句已被用户听到；可按人设简短接话，但不要机械复述。”播放进度快照、candidate 关联、这条提示与字/音素级位置均是设计占位，不属于 0.2.19 已实现范围。
...
- **待实现**：按 Worklet 播放进度与 candidate 绑定的一次性 interrupted UX；当前没有隐藏反馈，也不推断部分句文本。
- **待实验**：CosyVoice/Qwen 按各自官方能力实现真正音频流式下发；不得把整段音频再切块冒充真 streaming。
...
2. 固定 2 秒判停、LLM SSE、句级 audible history、有界有序 TTS consumer 与本地/CosyVoice managed identity 已完成测试切片；下一步优先做按播放进度绑定的一次性 interrupted UX，再分别验证 CosyVoice、Qwen 的官方真流式 adapter，同时继续收集 trace 决定何时投入神经 VAD。
