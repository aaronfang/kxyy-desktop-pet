...
response_done     { responseId, generation, status }
```

音频 chunk、assistant 文本和阶段结束 sentinel 均应关联 `responseId` 和 generation。取消后，任何旧 generation 输出都必须被丢弃；新 generation 不受旧 discard window 影响。
...
- 定义 speech onset、candidate、confirmed、ASR final、LLM first token、TTS first audio、audio played 时间戳。
- 建立固定 PCM 回放集：正常说话、短停顿、咳嗽、键盘、外放回声、连续插话。
- 为取消域、迟到 ASR、旧音频丢弃、误候选恢复和断线清理补纯状态机测试。
