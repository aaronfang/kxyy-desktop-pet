...
可恢复播放与候选让声已经有测试版：本地链路检测到疑似人声会立即发 candidate 并由前端 duck/暂停，约 1.05 秒 soft-end/reopen 后才提交整段 Whisper。0.2.17 为本地/CosyVoice 稳定句加入实际播放完成回执；0.2.18 又把 LLM 消费、句级合成和有序播放拆成有界阶段，Windows/Linux Qwen 可让下一句合成与当前句播放重叠，CosyVoice 最多并行合成两句；macOS MLX 为避免阻塞共享池中的插话 ASR，播放期间不预取。当前主要瓶颈仍是 confirmed 等待句尾与整段 ASR，以及每个稳定句的音频仍需整段合成；神经 VAD、真实声学回放和真正音频流式仍是后续工作。
...
| 本地实时语音 | WebView PCM -> Python -> RMS VAD -> Whisper -> 桌面文字代理 -> TTS | 旁路采集、ASR 验证后打断、Qwen/CosyVoice、generation 取消、DeepSeek/Ollama 复用、LLM SSE、有界句级预取、有序播放与句级可听历史 | ASR 仍为整段；TTS 仍按稳定句整段合成；神经 VAD 与真正音频流式未实现 |
...
边界必须明确：规则波形只能证明状态机和阈值没有机械回归，不能代表真实人声、房间回声、神经 VAD 准确率或“30 分钟误打断 <= 1 次”体验指标。真实或公开许可录音的声学回放集仍待建立。generation-tagged `CancelScope`、文字 provider、LLM SSE/稳定句、句级可听历史和有界有序 TTS consumer 已在 0.2.14–0.2.18 依次补齐；下一项基础工作是下行音频 identity 与 interrupted UX，之后才进入后端专用真流式实验。
...
纯状态 `StableSentenceBuffer` 以 `。！？!?；;` 和换行为强句末，达到 40 字时优先在 `，,、：:` 切分，60 字无断点时硬切，内部缓冲始终有界；短尾段在流结束时 flush。每个有效 content delta 立即作为 `assistant` delta 发送，首个稳定句在完整回复结束前触发现有 TTS；`tts_start` / `tts_end` 显式包住本轮后端音频生产期，前端只有在生产结束且播放 ring 已 drain 后才记录 response completed。Qwen/CosyVoice 仍是“每个稳定句整段合成 -> 既有 PCM chunk 发送”，**不是真正音频流式**。0.2.16 当时的 consumer 会在逐句合成和发送期间停止消费后续 token；该限制已由 0.2.18 的有界有序 consumer 缩小，但队列饱和时仍会反压。
...
`assistant_end` 现在表示文字流结束，可早于最后一句音频；`tts_end` 仍只在所有已接纳句子按序发送完后出现。因此 `llm_response` trace 通常不再包含普通播放 pacing，但句队列满时仍会包含有界反压，不能当作供应商纯模型耗时。此版仍是**逐句整段 TTS**，不是 Qwen/CosyVoice 真音频 streaming，也没有真实 trace 证明 TTFA 或句间空隙改善。固定测试覆盖倒序合成仍有序播放、Windows/Linux Qwen 播放/合成重叠、macOS MLX 播放期不预取、队满反压、取消/异常清理、无效/奇数/过长 PCM，以及 segment/history 顺序；真实体验结论仍须用本地模型和 CosyVoice 账户手测。
...
| TTS | Qwen 串行/Cosy 最多双路句级合成；单路有序 PCM | 句子合并、流式 chunk、TTFA/RTF | 后续引入真正 Qwen/Cosy 音频流式输出 |
...
| 测试 | reducer、合成 PCM、SSE、分句、有界 TTS 顺序/取消；真实许可录音待补 | VAD、取消、生命周期、TTS 合并测试 | 下一步补音频 identity，再补真实声学回放和体验指标 |
...
- **待实现**：managed downlink 音频 identity、Qwen/CosyVoice 真正音频流式下发；字/音素级中断位置与按播放时长触发的隐藏反馈仍待实现。
...
2. 固定 2 秒判停、LLM SSE、句级 audible history 与有界有序 TTS consumer 已完成测试切片；下一步优先补 managed downlink 的 generation/segment/chunk identity 与 interrupted UX，再做后端专用真流式 adapter，同时继续收集 trace 决定何时投入神经 VAD。
