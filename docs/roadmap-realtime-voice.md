...
可恢复播放与候选让声已经有测试版：本地链路检测到疑似人声会立即发 candidate 并由前端 duck/暂停，约 1.05 秒 soft-end/reopen 后才提交整段 Whisper。0.2.17–0.2.20 依次补齐可听历史、有界句级管线、`managed-v1` 下行身份与 candidate-bound 临时打断提示；0.2.21 又让 CosyVoice + Worklet 在独立协商后直接请求 24k raw PCM，provider binary 到达即以单路有界 sender 下发。Qwen、legacy、旧服务与火山保持原路径。当前主要瓶颈仍是 confirmed 等待句尾与整段 ASR；Qwen 真流式、神经 VAD、真实声学回放和字/音素级恢复仍是后续工作。CosyVoice 的真实 TTFA、字节序与听感仍需当前账号实测，不能只凭确定性 fake transport 宣称指标改善。
...
| 本地实时语音 | WebView PCM -> Python -> RMS VAD -> Whisper -> 桌面文字代理 -> TTS | 旁路采集、ASR 验证后打断、Qwen/CosyVoice、generation 取消、DeepSeek/Ollama 复用、LLM SSE、有界句级队列、有序播放、句级可听历史、`managed-v1` 下行身份、Worklet-only interrupted hint、CosyVoice 原生 PCM 流式 adapter | ASR 仍为整段；Qwen TTS 仍按稳定句整段合成；神经 VAD、字/音素级恢复未实现；CosyVoice 真实账号指标待实验 |
...
只有 CosyVoice + 播放 Worklet 的新前端会在 `managed-v1` 之外提供 `ttsStream:["provider-pcm-v1"]`；Python 只有配置了流式 adapter、已选择 `managed-v1` 且客户端显式提供该能力时才回显。Qwen、legacy、旧前后端、未协商会话与火山保持原有整句/原始 PCM 路径，KXAU 24-byte header 和火山协议常量均未改变。HTTP 朗读仍返回完整 MP3。
...
**待实验**：官方格式枚举与 browser demo 支持 24k mono 16-bit PCM，但公开材料没有规范性写明字节序；当前按官方 demo 的 `Int16Array` 消费方式和项目目标解释为桌面 s16le。合入/发布前仍需用当前账号对照 WAV 或已知短音频，确认字节序、音高、语速、首包延迟、句间空隙、取消计费与 v3.5 复刻音色兼容。没有真实 trace 前不能宣称 p95 或听感改善。Qwen 真流式、Cosy 多句流式预取、字/音素恢复、神经 VAD 与 3 秒 ring 收紧均不属于本版。
...
| 测试 | reducer、合成 PCM、SSE、分句、有界 TTS 顺序/取消、managed identity、candidate-bound interrupted hint；真实许可录音待补 | VAD、取消、生命周期、TTS 合并测试 | 下一步分别补后端真流式与真实声学指标 |
...
- **待实验**：先实测 CosyVoice PCM adapter，再按 Qwen 官方能力实现本地真流式；不得把整段音频再切块冒充真 streaming。
...
1. 播放 Worklet、两阶段打断测试版和 candidate-bound interrupted UX 已完成；继续用 trace 与真实声学回放决定 VAD 调参时机。
2. 固定 2 秒判停、LLM SSE、句级 audible history、有界有序 TTS consumer、本地/CosyVoice managed identity、一次性 interrupted hint 与 CosyVoice PCM 真流式 adapter 已完成测试切片；下一步先用真实 CosyVoice 账号验证 PCM/TTFA，再实现 Qwen 官方真流式 adapter，同时继续收集 trace 决定何时投入神经 VAD。
