...
> 状态基准：2026-07-30，当前正式版为 `v0.2.46`。下面的完成度按“已发布 / 测试版或部分实现 / 待实现”描述，不使用主观百分比；Memory 与实时语音的实现边界分别以各自专项路线图为准。
...
| 人设与关系玩法 | **元元日常模式已实现，真实效果待体验验证** | 日常私聊卡片、条件直播 lore、会话 motif cooldown、逐轮时间、安全网页观察契约、深聊与关系/心情表现层边界 | 接入并审计一个真实搜索 provider；做真实模型对话与长期关系投影实验 |
...
**当前实测（2026-07-29）**：默认卡 v1.1.0 的 system prompt 为 7,638 个 Unicode code point，仍为 25 组/50 条 few-shot；卡片保留 5 组按需职业示例，但日常请求会过滤它们。prompt 中“直播”2 次、“直播间”0 次、“开播”1 次、“下播”2 次、“礼物”1 次、“火锅鸡”3 次（其中包含去重复与“不是当前饭菜”的约束，不代表默认回答）。文字每请求刷新时间；受管本地/CosyVoice 协商 `turn-local-v1` 后每轮刷新，旧服务和火山降级不变。网页观察接口与安全注入已实现但默认关闭，当前 provider 固定为 `none`，所以本版本不会实际联网搜索。
