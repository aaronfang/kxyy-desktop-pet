...
> 状态基准：2026-08-30，当前发布候选为 `v0.2.52`。下面的完成度按“已发布 / 测试版或部分实现 / 待实现”描述，不使用主观百分比；Memory 与实时语音的实现边界分别以各自专项路线图为准。

> 本轮本地 Qwen3-TTS 交付 6 个受 allow-list 和 SHA-256 校验的音色 preset（5 个自动评分最高参考音 + 固化为 `legacy-12s.wav/.txt` 的最早基线）；当前默认参考音仍由 `ref.wav/ref.txt` 提供。设置保存后下一句热加载，聊天 Debug 显示当前音色。75 条试听样例及其余候选只作本地评测，不进入资源包；语气表现力微调延期。
...
| 主动陪聊 | **本地实现完成，设备体验待测** | 本地/CosyVoice A-D：最终播放回执计时、四类短回应、暂停/恢复、有限 Memory 选题、确认态退让与 schema v9 无文本诊断 | 真实设备抢话率/等待感/连续倾听舒适度与火山能力门（E） |
...
- **人设（persona）**：默认"开心元元"由 `persona-cards/kxyy-yuanyuan/persona-card.json` 维护，并经开发明文/加密资源链路供 App 使用；用户导入的人格卡由 Rust 运行时加载。
- **当前二者仍独立，但人设已非单例**——切换桌宠形象不会自动切换人设；设置页已能导入、切换、导出和删除本地人格卡。元元默认卡的内容治理与上游同步约束见 2.1。
...
| 深聊触发已补充长表达判定 | `detectDeepIntent()` 保留明确求展开正则，并对 40 字以上、多分句、带个人处境与反思/情绪线索的表达开启深聊；URL、代码、图片元数据和纯命令块 fail closed。 | 召回面已扩大且有表驱动测试；它仍是确定性轻量判定，不宣称等同模型语义分类。 |

#### 2.1.1 元元日常人设改进基线与权威方案（2026-07-29）

本节是“默认元元怎么说话、知道什么、何时能声称什么”的权威路线。Memory 数据结构仍以 [Memory Brain 路线图](./roadmap-memory-brain.md) 为准，实时协议仍以 [实时语音路线图](./roadmap-realtime-voice.md) 为准。
...
**当前实测（2026-07-30；网络缓存实现复核于 2026-08-17）**：默认卡 v1.1.0 的 system prompt 为 7,638 个 Unicode code point，仍为 25 组/50 条 few-shot；卡片保留 5 组按需职业示例，但日常请求会过滤它们。prompt 中“直播”2 次、“直播间”0 次、“开播”1 次、“下播”2 次、“礼物”1 次、“火锅鸡”3 次（其中包含去重复与“不是当前饭菜”的约束，不代表默认回答）。文字每请求刷新时间；受管本地/CosyVoice 协商 `turn-local-v1` 后每轮刷新，旧服务和火山降级不变。近期话题缓存默认关闭，用户开启后由 Rust 预取固定中文来源并保存有界短期缓存；文字聊天提供 `relevant`、`occasional`、`active` 三档参与度，后两档分别每六/三个合适回合从本地缓存低频抽取一条线索。普通第一人称近况仍不触发环境话题；已注入来源使用会话内有界冷却，追问只回答新增问题，不重复上一轮整条观察。本地/CosyVoice 继续沿用能力协商后的按需路径。Tavily 仍需用户显式选择并填写本地 API Key，仅作为明确搜索意图的按需增强。请求与缓存都不保存整页正文；Rust 与前端分别限制响应、URL、来源时间、正文和最终注入长度，任何异常都降级为空观察且不阻塞普通对话。

**目标分层**：

1. **不可变身份与语言风格**：保留姓名、主播职业、东北表达、性格和经核验的个人经历；直播是职业背景，不是默认会话现场。
2. **默认日常私聊模式**：默认以朋友间当下聊天为场景；只有用户明确问直播、当前话题明确属于直播，才按需注入直播知识。不得仅凭日程推断“正在直播”。
3. **逐轮时间上下文**：文字每轮注入设备本地日期、星期、时间和时区；本地/CosyVoice 在能力协商后逐轮刷新。天气和位置必须来自用户授权与检索，不能从居住地推断。
4. **外部事实观察层**：对新闻、热点、赛事、天气等时效问题，由 App 在模型请求前执行受限只读检索，传入有来源和抓取时间的非可信观察；模型不得把观察当指令，检索失败必须明确“不确定/没查到”。外部观察默认不进入 Memory。
5. **关系与短期心情**：慢关系只能由有来源、可查看和可纠正的互动事件投影；快心情只调制语气、称呼、主动性、动作和 `SpeechStyle`。两者都不得改写 system/persona 事实。

**内容与认识边界**：

- “火锅鸡”是多个饮食偏好之一，不是每次被问“吃什么”的默认答案；区分“喜欢吃什么”与“今天正在吃什么”。不知道角色当前饭菜、活动或私生活状态时不得编造。
- 对火锅鸡、主播梗和其他高频锚点使用有界、仅当前会话的 motif cooldown；它不进入长期记忆，也不永久禁止用户主动提起的话题。
- 个人事实只来自人格卡；当前外部事实只来自带来源观察；稳定常识可由模型回答但应表达不确定性；当前私生活状态未知即未知。
- 深聊保留元元口吻，但有可靠外部材料时应给出具体信息与来源，不把严肃问题强行改写为空泛安慰或虚构亲历。

**实施顺序与状态**：

| 优先级 | 切片 | 可验收结果 | 状态 |
|---|---|---|---|
| P0-A | persona 纯函数测试；拆分纯时间与条件直播上下文 | 默认上下文含准确时间但不含直播阶段、倒计时、礼物；明确直播问题仍可取直播 lore | `completed` |
| P0-B | 重写默认卡规则和 few-shot；食物语义与 motif cooldown | 默认主动提示无直播间话术；重复询问不再固定落到火锅鸡；未知当前饭菜不编造 | `completed` |
| P0-C | provider-neutral 时间上下文；本地/CosyVoice 逐轮刷新 | 文字请求逐轮刷新；受管实时每轮刷新；旧服务和火山保持兼容降级 | `completed` |
| P1-D | 受限只读网页检索接口与显式设置 | 结果有来源/时间/大小上限，不可执行、不自动入 Memory；超时、无结果和关闭时 fail closed | `completed`（安全契约与假 adapter）；真实 provider/凭据 pending |
| P1-E | 语义深聊；慢关系/快心情调制边界 | 长表达可稳定触发深聊；状态只能影响允许的表现层，不能改变人格事实 | `completed`（配置关系 + 本轮心情表现层）；自动关系演化仍走 Memory 后续路线 |

P1-D 的 connector 权限、认证、scope、审计与撤销原则复用 [Memory Brain M5](./roadmap-memory-brain.md#m5综合大脑与外部工程接入)，但网页临时观察与用户授权 connector 数据必须保持不同来源类型；本路线不重复定义 Memory schema。
...
| 人格卡已有本地配置化流程，但生态规范仍不完整 | 设置页已支持导入、切换、导出和删除人格卡，用户卡无需重新编译；当前缺少兼容性声明、素材/声纹授权检查、版本迁移和社区分发规范。 | 技术入口已打通，下一瓶颈是卡片质量、供应链与兼容治理，而不是再造一套加载器。 |
...
| **人格卡生态治理** | 复用现有 JSON Schema v1 和设置页导入/切换能力，补充兼容版本、来源与素材许可、迁移失败回滚、导入大小上限和社区分发说明；默认内置卡继续加密，用户卡保持本地文件。 | 已无需重做多人设加载；把后续投入集中到可验证、可迁移、可分享且不误带未授权素材的人格卡生态。 | `persona-cards/`、schema/validator、导入校验与文档 | 中 |
...
| **语义化深聊触发（已实现）** | `detectDeepIntent()` 保留明确求展开正则，并补充 40 字以上、多分句、含个人处境和反思/情绪线索的确定性判定；URL、代码、图片元数据和纯命令块不触发。 | 提升深聊召回，同时避免把粘贴内容误当倾诉；表驱动测试已覆盖。 | `src/ai/persona.js::detectDeepIntent` | 小 |
| **淡化直播场景、切换为"主播日常聊天"模式（已实现）** | `computeLiveContext()` 默认只提供日期、星期、时间、时区和日常状态；用户明确谈主播工作时才附加有界 lore，并禁止由日程推断当前直播状态。卡片和主动提示也已迁移到朋友式日常私聊。 | 默认对话不再被直播阶段驱动，主播身份与工作资料仍可按需使用。 | `src/ai/persona.js::computeLiveContext`、默认人格卡 | 中 |
...
| **淡化直播场景、切换为"主播日常聊天"模式** | 3.1 | 默认卡、few-shot 过滤、主动提示与时间上下文均已迁移；明确职业问题仍可访问有界 lore | **中** | `- [x]` ✅ 2026-07-29 |
...
| **`computeLiveContext` 时间/职业语境解耦** | 新增 | 默认只输出时间与日常私聊模式；显式职业意图才读取 `lore`，且不推断当前开播状态 | 中 | `- [x]` ✅ 2026-07-29 |
...
| **后台陪伴通话（VoxCPM2）** | [独立规格](./spec-background-companion-call.md) | 独立会话类型；默认RightAlt按住说话、空闲不采集PCM、60秒安全结束/3段续说、20分钟低频主动性和5分钟恢复。复用managed playback/Audible History，但不属于4.7的第四档，也不观察屏幕或系统音频 | 中 | `- [ ]` 已完成产品决策，待按M0--M6实现与双平台实机验收 |
