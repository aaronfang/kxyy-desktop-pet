...
| 实时语音通话 | 火山端到端实时语音大模型 | 本地 Qwen3-TTS + Whisper + 当前文字 provider / CosyVoice 通义云桥接 | `realtime.rs`（火山）或本机 Python WS（本地/CosyVoice） | 0.2.15 起复用 `textProvider`；0.2.18 起使用有界句级预取与严格有序播放；0.2.19 起本地/CosyVoice 协商 managed 下行身份，火山仍走 raw PCM；朗读与通话**共用同一个语音后端选择** |
...
| 无自动化测试/无 lint | `CLAUDE.md` 明确"There is no test suite and no linter configured"。 | 任何涉及 `persona.js` 拆条/记忆/主动性判定这类**纯函数逻辑**的改动，只能靠手测覆盖，回归风险随功能增多而上升。 |
...
| **打断后的自然对话上下文** | 0.2.17 已记录完整播完的句段，0.2.19 已补本地/CosyVoice chunk identity 地基；仍待按播放进度与 candidate 绑定，只在确实播出超过约 1 秒且句中被截断时注入一次性隐藏提示，不默认让角色每次都抗议。 | 避免模型把用户未听到的尾句视为已知，也避免机械化“你打断我”话术。 | 详见 [实时语音专项路线图](./roadmap-realtime-voice.md) 4.6 | 中 |
...
| **两阶段自然打断与播放缓冲** | 3.3 | AudioWorklet ring、candidate/confirmed/rejected 已有测试版；仍待真实声学回放与神经 VAD 调参 | 中 | `- [~]` 🔧 测试版（0.2.11+） |
| **SenseVoice final ASR + SER 实验** | 3.3 | 先验证中文识别、情绪标签、加载资源和与 VAD 的组合；不直接替换默认 Whisper | 低 | `- [ ]` |
| 结构化关系画像编辑器 | 3.1 | 无 | 低 | `- [ ]` |
| 打断后的可听上下文与反馈话术 | 3.3 | 0.2.17 已有句级 completed-segment history，0.2.19 已补本地/CosyVoice managed identity；播放进度/candidate 关联、字/音素位置与按时长触发的隐藏反馈仍待补 | 低 | `- [~]` 🔧 identity 地基完成 |
...
6. **无测试覆盖下的回归风险**
   `persona.js` 里的纯函数（`splitReply`/`shouldDoFollowup`/`detectDeepIntent` 等）是本报告多项功能的共同修改目标。在完全没有自动化测试的现状下，多人协作或多轮迭代容易出现"改好感度逻辑时不小心破坏了拆条逻辑"这类隐性回归。建议至少覆盖这批纯函数的单测后，再推进 P1/P2 功能。
