...
| 实时语音通话 | 火山端到端实时语音大模型 | 本地 Qwen3-TTS + Whisper + 当前文字 provider / CosyVoice 通义云桥接 | `realtime.rs`（火山）或本机 Python WS（本地/CosyVoice） | 0.2.15 起复用 `textProvider`；0.2.18 起使用有界句级预取与严格有序播放；朗读与通话**共用同一个语音后端选择** |
...
| 结构化关系画像编辑器 | 3.1 | 无 | 低 | `- [ ]` |
| 打断后的可听上下文与反馈话术 | 3.3 | 0.2.17 已有句级 completed-segment history；字/音素位置与按时长触发的隐藏反馈待补 | 低 | `- [~]` 🔧 句级基础完成 |
