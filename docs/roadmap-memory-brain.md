...
- [ ] 先建立固定离线召回集和纯 FTS 基线，再决定 embedding 是补召回还是只做 rerank；验收记录跨表达召回率、无关误召回率、P95、内存和包体积，不以单次观感代替评测。
- [ ] 本地模型默认可选且不得自动下载；模型、tokenizer、许可证、版本、字节大小和 SHA-256 必须锁定，安装/更新沿用 VAD/ASR 的 staging、验证、ready-marker-last 和可恢复切换原则。
- [ ] embedding 以 `model_sig` 隔离，后台回填必须有界、可取消、可重建；模型不可用、向量缺失或版本切换时静默退回 FTS，不阻塞聊天和删除。
- [ ] 远程 embedding 必须逐项征得用户同意，不默认上传记忆正文。
- [ ] 图片、音频和文档使用 content-addressed artifact 引用，不把大文件塞进 SQLite。

参考说明：Ackem 已验证了本地 BGE-small、`model_sig` 持久化、FTS/向量混合和无 embedding 降级的产品价值，但其具体模型管理、解压、下载和多路召回实现不作为本工程代码基线。本项目仅借鉴机制，按自身供应链、事件、scope、删除一致性和测试门槛 clean-room 实现。
